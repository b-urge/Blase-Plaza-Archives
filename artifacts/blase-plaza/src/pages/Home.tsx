    ...
    mutation: {
      onSuccess: () => {
        queryClient.invalidateQueries({
          queryKey: getGetSurveyStatsQueryKey(),
        });
  ...
  return (
    <div className="win98-window p-6 h-full">
      <h2 className="text-xl font-bold mb-2">
        WELCOME TO THE BLASÉ PLAZA ARCHIVES
      </h2>
      <hr />

      <p className="mb-6 leading-relaxed">
        This database serves as the central repository for field surveys of
        commercial plazas in Broward County slated for demolition or significant
        redevelopment.
      </p>

      <div className="win98-window-inset p-4 mb-6">
        <h3 className="font-bold mb-3 uppercase border-b-2 border-[#c0c0c0] pb-1">
          Database Statistics
        </h3>
        {isLoading ? (
          ...
          <ul className="space-y-2 text-sm">
            ...
            <li className="mt-3 pt-3 border-t border-[#c0c0c0]">
              <strong>BY STATUS:</strong>
            </li>
            <li className="pl-2">
              Demolition Pending:{" "}
              {(stats as any).byStatus?.["Demolition Pending"] ?? 0}
            </li>
            <li className="pl-2">
              Renovation Pending:{" "}
              {(stats as any).byStatus?.["Renovation Pending"] ?? 0}
            </li>
            <li className="pl-2">
              Declining: {(stats as any).byStatus?.["Declining"] ?? 0}
            </li>
            <li className="pl-2">
              Post-Intervention:{" "}
              {(stats as any).byStatus?.["Post-Intervention"] ?? 0}
            </li>
          ...
          <p className="text-sm text-[#cc0000]">
            SYSTEM ERROR: Unable to load statistics.
          </p>
        )}
      </div>
      ...
      <div className="mt-8 p-4 bg-[#d4d0c8] border-2 border-[#808080]">
        ...
        <p className="text-xs mb-3">
          Only use these functions for manual data synchronization and
          initialization.
        </p>
        ...
        {(seedMutation.isSuccess || syncMutation.isSuccess) && (
          <p className="text-[#008000] text-xs mt-2 font-bold">
            Operation completed successfully.
          </p>
        )}
        {(seedMutation.isError || syncMutation.isError) && (
          <p className="text-[#cc0000] text-xs mt-2 font-bold">
